...
```{r} 
gam_fit <- mgcv::gam(signal ~ s(lepton.pT) + s(lepton.eta) + s(missing.energy.magnitude) + s(missing.energy.phi) + s(jet.1.pt) + s(jet.1.eta) + s(jet.1.phi) + s(jet.2.pt) + s(jet.2.eta) + s(jet.2.phi) + s(jet.3.pt) + s(jet.3.phi) + s(jet.4.pt) + s(m_jj) + s(m_jjj) + s(m_lv) + s(m_jlv) + s(m_bb) + s(m_wbb) + s(m_wwbb), data = train)
...
plot(svm_fit, data = svm_train, formula = m_bb ~ m_wwbb)
```